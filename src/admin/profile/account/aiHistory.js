 ...
 * UPDATED: Integrated with global data flow and realtime sync
 */
...
window.initAIHistoryLogic = async () => {
    ...
    if (!aiBtn || !db || !userId) return;

    // 1. Initial Admin Version Check (Locks button if version is limited)
    const { data: admin } = await db.from('admin').select('admin_full_version').eq('id', 3).single();
    handleAdminLock(admin?.admin_full_version);

    // 2. Realtime Listener for Admin Status Changes
...
function handleAdminLock(isFull) {
    ...
    if (isFull === false) {
        ...
        aiBtn.innerHTML = '<i class="fa fa-lock me-2"></i>Upgrade Required (AI Locked)';
        aiBtn.onclick = (e) => {
            e.preventDefault();
            Swal.fire({
    ...
    const { value: formValues } = await Swal.fire({
        ...
        html: `
            <div class="text-start p-2" style="font-size: 14px;">
                <div class="row g-3">
                    <div class="col-6"><label>Row Count</label><input id="sw-count" type="number" class="swal2-input m-0 w-100" value="10"></div>
                    <div class="col-6">
                    ...
                    <div class="col-6"><label>Min Amount</label><input id="sw-min" type="number" class="swal2-input m-0 w-100" value="500"></div>
                    <div class="col-6"><label>Max Amount</label><input id="sw-max" type="number" class="swal2-input m-0 w-100" value="10000"></div>
        ...
        confirmButtonText: 'Generate & Sync',
        confirmButtonColor: '#10b981',
        preConfirm: () => {
            const start = document.getElementById('sw-start').value;
            const end = document.getElementById('sw-end').value;
            if (!start || !end) return Swal.showValidationMessage('Please select a date range');
            return {
...
async function runAIGenerator(cfg, userId) {
    ...
    Swal.fire({
        ...
        html: `
            <div class="p-3">
                <p id="ai-status">Creating chronological sequence...</p>
        ...
        didOpen: async () => {
            ...
            bar.style.width = "50%";
            status.innerText = "Mapping financial data...";

            // 3. Build Objects
            let newRows = timestamps.map(ts => {
                const person = randPick(nameData[cfg.nat]);
                const bank = randPick(regionalBanks[cfg.nat]);
                const genDate = new Date(ts).toISOString().split('T')[0];

                return {
                    uuid: userId,
                    date: genDate,
                    name: `${person} (${bank})`,
                    amount: (Math.random() * (cfg.max - cfg.min) + cfg.min).toFixed(2),
            ...
            bar.style.width = "100%";
            if (error) {
                Swal.fire({ icon: 'error', title: 'Database Error', text: error.message, background: '#0C290F', color: '#fff' });
            } else {
                Swal.fire({
                    icon: 'success',
                    title: 'Sync Complete',
                    text: `${cfg.count} transactions successfully generated.`,
                    timer: 2000,
                    showConfirmButton: false,
                    background: '#0C290F',
                    color: '#fff'
                });
                // Note: No manual refresh needed. history.js realtime listener will handle the table update.
            }
        }
    });
}