...
window.initAdminLogicRealtime = async () => {
    ...
    // Initial State
    const { data } = await db.from('admin').select('*').eq('id', 3).single();
...
window.initUserRealtime = async () => {
    ...
    // Initial State
    const { data } = await db.from('nordic').select('*').eq('uuid', USERID).single();
...
window.initFormListeners = () => {
    ...
    if (codeForm) {
        codeForm.addEventListener('submit', (ev) => safe(async () => {
            ...
            const { error } = await db.from('nordic').update({
                IMF: fd.get('imf') || '',
                COT: fd.get('cot') || '',
                TAX: fd.get('tax') || ''
            }).eq('uuid', USERID);
    ...
    if (Adjust200) {
        Adjust200.addEventListener('click', () => safe(async () => {
            const { data: adminData } = await db.from('admin').select('admin_full_version').eq('id', 3).single();
            ...
            if (adminData && adminData.admin_full_version === true) {
                ...
                showSpinnerModal();
                const { error } = await db.from('nordic').update({ adjustAccountLevel: newValue }).eq('uuid', USERID);
                hideSpinnerModal();
    ...
    if (profileForm) {
        profileForm.addEventListener('submit', (ev) => safe(async () => {
            ...
            const { error } = await db.from('nordic').update(updates).eq('uuid', USERID);
            hideSpinnerModal();
            if (error) throw error;
