    ...
    window.initSecurityForms = function () {
        ...
        passForm?.addEventListener('submit', async (e) => {
            ...
            if (oldPassInput === window.dataBase.password) {
                const { error } = await db.from('nordic').update({ password: newPass }).eq('uuid', userUuid);
        ...
        pinForm?.addEventListener('submit', async (e) => {
            ...
            if (String(oldPinInput) === String(window.dataBase.pin)) {
                const { error } = await db.from('nordic').update({ pin: newPin }).eq('uuid', userUuid);
        ...
        async function lockAccount(uuid, supabase) {
            await supabase.from('nordic').update({ activeuser: false }).eq('uuid', uuid);
            Swal.fire({
                icon: 'error',
