    ...
    if (deleteAllBtn) {
        deleteAllBtn.onclick = async () => {
            ...
            if (confirmPurge.isConfirmed) {
                ...
                try {
                    const r = await fetch(`https://bank-app-api-cyan.vercel.app/api/admin-chat?purge_all=true&user_uuid=${activeChatSessionUserUuid}`, {
                        method: "DELETE",
...
async function executeUpdateSingleChatMessage(msgId, newText) {
    const adminToken = localStorage.getItem("admin_session_token");
    try {
        const response = await fetch("https://bank-app-api-cyan.vercel.app/api/admin-chat", {
            method: "PUT",
            headers: {
...
async function executeDeleteSingleChatMessage(msgId) {
    const adminToken = localStorage.getItem("admin_session_token");
    try {
        const response = await fetch(`https://bank-app-api-cyan.vercel.app/api/admin-chat?message_id=${msgId}`, {
            method: "DELETE",
            headers: { "Authorization": `Bearer ${adminToken}` }
        });
...
async function fetchSecureConversationStreams(isInitialLoad = false) {
    ...
    try {
        const r = await fetch(`https://bank-app-api-cyan.vercel.app/api/admin-chat?uuid=${activeChatSessionUserUuid}&page=1&limit=${chatMaxLimitPerPage}`, {
            method: "GET",
            headers: { "Authorization": `Bearer ${adminToken}` }
        });
...
async function fetchOlderHistoricalChatLogs() {
    ...
    try {
        const response = await fetch(`https://bank-app-api-cyan.vercel.app/api/admin-chat?uuid=${activeChatSessionUserUuid}&page=${nextPage}&limit=${chatMaxLimitPerPage}`, {
            method: "GET",
            headers: { "Authorization": `Bearer ${adminToken}` }
        });
...
async function clearFileAssetStorageUpload(file) {
    ...
    try {
        const response = await fetch("https://bank-app-api-cyan.vercel.app/api/avatar", {
            method: "POST",
            headers: {
                "Authorization": `Bearer ${adminToken}`,
                "X-Action": "chat",
